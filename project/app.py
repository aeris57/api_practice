...
def load_user():
    if 'screen_name' in session:
        g.user = User.load_from_db_sname(session['screen_name'])
...
def twitter_auth():
    ...
    user = User.load_from_db_sname(access_token['screen_name'])
    if not user:
...
def search():
    ...
    return render_template('search.html', contents=tweet_texts)
